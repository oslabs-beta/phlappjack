...
router
	.get('/accounts', async (ctx) => {
  	const accounts_findAll = await accounts.find( 
  		{_id: { $ne: null }},
...
.delete('/accountsdsjhdsjkchjdscshjkch:id', async (ctx) => {
  const { limit } = helpers.getQuery(ctx, {mergeParams: true });
	console.log('deleteone works',limit)
  if( limit ){
...
.delete('/accountsstuff', async (ctx) => {
  // const { limit } = helpers.getQuery(ctx, {mergeParams: true });
	console.log('test')
  // if( limit ){
  // 	const accounts_deleteMany = await accounts.deleteMany(
